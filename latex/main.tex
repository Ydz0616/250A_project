...
So far, our hypothesis is that HMM will outperform rule-based baseline and baseline models without sequential information. On top of this, we will compare the performance variation in different HMM structures, mainly, we would compare the performance between ordinary HMM and edge-emitting HMM, where the transition between two consecutive trip modes is used as a hidden state.

We will use a binary accuracy function to measure the model’s performance, where the predicted purpose is compared to the ground truth purpose with an indicator function.
